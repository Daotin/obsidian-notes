...
> [!note] ❓ 在 package.json 中，main 和 bin 的作用和区别分别是什么？
> 
...
> 
> 因此，"main"属性用于指定模块的入口文件，而"bin"属性用于指定命令行工具的入口文件。
...
// #! 是用于指定脚本的解释程序
#! /usr/bin/env node 告诉不同的系统，从哪里查找node执行文件
console.log("hello world");
```

#### 1.3 npm link 链接到全局（或者直接node执行也是一样）

库包在开发或迭代后，不适合发布到线上进行调试（过程繁琐且会导致版本号膨胀）。

`npm link`可以帮助我们模拟包安装后的状态，它会在系统中做一个快捷方式映射，让本地的包就好像 install 过一样，可以直接使用。可以用`npm unlink`解除链接。
...
> 
> 1. 在 ESModule 中，全局变量 `__dirname` 未定义。因此，需要导入 `fileURLToPath` 和 `dirname` 函数来创建 `__dirname` 变量，表示当前文件所在的目录。
> 
...
| [ora](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fora)                                          | 控制台 loading 样式（或者`progress-estimator`加载动画条，可以预估git clone的下载时间） |
| [figlet](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Ffiglet)                                    | 控制台打印 logo（或者使用`log-symbols`）                                  |
| [easy-table](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Feasy-table)                            | 控制台输出表格                                                        |
| [download-git-repo](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fdownload-git-repo)              | 下载远程模版（或者使用`simple-git`）                                       |
...
此时就有了`command-cli`命令使用的说明信息，在命令行中执行一下
...
[chalk](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fchalk)可以美化我们在命令行中输出内容的样式，例如对重点信息添加颜色，我们在 command-cli 项目中进行完善。
...
我们已经对脚手架的基本雏形和常用的工具库都有所了解了，现在我们就来搭建自己的脚手架吧。
...
这里我们拿 vuejs 远程仓库为例（或者使用你自己的远程仓库的项目模版）。
...
3. npm config set registry 设置为官方的源
...
3. npm login登录
4. 使用 `npm publish`将项目发布至 npm，更新的时候，注意修改版本号（使用npm version patch）。
