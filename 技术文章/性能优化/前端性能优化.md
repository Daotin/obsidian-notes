...
1、首先打开“[Lighthouse](https://github.com/GoogleChrome/lighthouse)”面板

看：LCP，FCP等指标，是否满足标准指标。

2、打开浏览器的开发者工具，查看 **`Network`** 标签，优化首屏加载的资源。

> 首先看传统的`DOMContentLoaded` 和 `load` 的时长。

对于 Vue SPA，重点关注 Web Vitals 指标和用户真实体验指标，而不是传统的 DOMContentLoaded/load 时间。但是如果DOMContentLoaded和load差距过大，考虑是bundle过大，以及资源过大的问题。

- 统计接口耗时，看是否是接口慢的原因？（可以参考：[通过Timing瀑布图分析前端性能问题](通过Timing瀑布图分析前端性能问题.md)）
...
3、用 **`Vue Devtool`** 记录组件耗时，看是否是组件耗时过长

4、除此之外，还有**Performance API**和**web-vitals**
...
> web-vitals 缺点：有一定的兼容性问题。
> 
> Web-vitals 代码已经测试并通过，在所有主要的浏览器（包括 Internet Explorer 9 及以上版本）中可以正常运行。不过，用于捕获这些指标的一些 API 目前仅在基于 Chromium 的浏览器中可用（例如 Chrome、Edge、Opera、Samsung Internet）。
> 各项功能的浏览器支持情况如下：
> onCLS()：Chromium
> onFCP()：Chromium、Firefox、Safari
> onFID()：Chromium、Firefox（已弃用）
> onINP()：Chromium
> onLCP()：Chromium、Firefox
> onTTFB()：Chromium、Firefox、Safari


5、开启 `app.config.performance = true` 将会开启 Vue 特有的性能标记，标记在 Chrome 开发者工具的性能时间线上。
...
本地优化原则：
1. 先用 **Lighthouse** 进行整体分析，找到低分项（Lighthouse也是根据衡量标准走的，先做一遍粗优化）
2. 然后用 **Performance 面板** 找到具体性能瓶颈（如渲染、JS 阻塞）
3. 最后，再使用 **PerformanceObserver API/Web-vitals** 监测关键指标

线上监控原则：
1. 通过 **PerformanceObserver API** 收集真实用户体验数据
2. 定期使用 **Lighthouse** 进行回归测试
...
- 使用**按需引入**的依赖（比如使用 lodash-es 替代 lodash）
...
**5、[BlueLink项目打包体积优化](BlueLink项目打包体积优化.md)**
